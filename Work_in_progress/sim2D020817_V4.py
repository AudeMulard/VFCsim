...
nx = 130 #number of controle volume
dy = 0.3
ny = 200
...
parameters.writerow(['U']+['Mobility']+['epsilon']+['l']+['dx']+['nx']+['dy']+['ny']+['startpoint']+['amplitude'])
parameters.writerow([U]+[Mobility]+[epsilon]+[l]+[dx]+[nx]+[dy]+[ny]+[startpoint]+[amplitude])
...
#    phi.setValue(1-0.5*(1-numerix.tanh((x-nx*dx/2)/(2*numerix.sqrt(M*2*epsilon**2/l)))))
    for i in range(ny):
        a = numpy.random.normal(startpoint, amplitude)
...
viewer2 = Viewer(vars = (xVelocity), datamin=0., datamax=0.03)
viewer3 = Viewer(vars = (yVelocity), datamin=0., datamax=0.01)
