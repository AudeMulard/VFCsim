...
#        a = random.gauss(0.1, 0.005)
        ...
        phi.setValue(1-0.5*(1-numerix.tanh((x-a)/(2*numerix.sqrt(M*2*epsilon**2/l)))), where=(y<2*(i+1)*dy) & (y>2*(i*dy)))
...
#        ap[:] = xmat.takeDiagonal()
        ...
        xVelocity[nx-1]=U
    elapsed +=timeStep
    viewer.plot(filename="phi%d_90.png" % elapsed)
    viewer2.plot(filename="XVelocity%d_90.png" % elapsed)
    viewer4.plot(filename="pressure%d_90.png" % elapsed)
    viewer3.plot(filename="YVelocity%d_90.png" % elapsed)
    TSVViewer(vars=(phi, xVelocity, yVelocity, pressure,beta)).plot(filename="essaidonne%d_90.tsv" % elapsed)
