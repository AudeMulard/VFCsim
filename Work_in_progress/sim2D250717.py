...
#        phi.setValue(1-0.5*(1-numerix.tanh((x-a*nx*dx)/(2*numerix.sqrt(M*2*epsilon**2/l)))), where=(y<(i+1)*dy) & (y>(i*dy)))
        phi.setValue(1-0.5*(1+numerix.tanh((x-a)/(2*epsilon))), where=(y<(i+1)*dy) & (y>i*dy))
...
pressureCorrectionEq = DiffusionTerm(coeff=coeff) - velocity.divergence + k*phi
...
displacement = 20.
timeStep = 0.8 * dx / k #less than one space step per time step
elapsed = 0.
